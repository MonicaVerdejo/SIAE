...
if (array_key_exists("email2", $_POST)) {
    $db = new DB();
 	  $consulta = $db->connect()->prepare('SELECT correo FROM administrador WHERE correo = :email');
     ...
     $token = uniqid();
    $consulta = "UPDATE administrador set Token = '$token' WHERE correo = '{$resultado['correo']}'";
...
try {
    ...
    $mail->Body = 'Haga click en este <a href="http://localhost/SIAE2/admin/set/cambia_passwordAdmin.php?token='.$token.'">link</a> para cambiar su contraseña de usuario. <br> Gracias por tu tiempo y suerte trabajando con nosotros. ';

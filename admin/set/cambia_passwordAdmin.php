...
if (array_key_exists("token", $_GET)) {
	$consulta = $db->connect()->prepare('SELECT id, correo FROM administrador WHERE Token = :token');
	$consulta->bindParam(':token', $_GET['token']);
	$consulta->execute();

	if ($resultado = $consulta->fetch(PDO::FETCH_ASSOC)) {
		$consulta = "UPDATE administrador SET Token = NULL WHERE id = '{$resultado['id']}'";
		$db->connect()->exec($consulta);
	}
} else if (array_key_exists("uid", $_POST)) {

	$newpassword = password_hash($_POST['newPass'], PASSWORD_BCRYPT);
	$consulta = $db->connect()->prepare("UPDATE administrador SET password = '$newpassword' WHERE id ='{$_POST['uid']}'");
	$consulta->execute();
...
<body  style="background-color: #dfdfd9;">
	<section class="content-seccion">
		...
		<div class="content-form">
			...
			<form method="post" action="cambia_passwordAdmin.php" name="signup">
